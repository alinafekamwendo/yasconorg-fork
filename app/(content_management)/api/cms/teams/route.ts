...
import type { CmsTeamType } from "@/lib/cms/constants";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const type = searchParams.get("type") as CmsTeamType | "all" | null;
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!body.name || !body.role || !body.focus || !body.teamType || !body.region) {
      return NextResponse.json({ error: "name, role, focus, teamType, region are required" }, { status: 400 });
    }
