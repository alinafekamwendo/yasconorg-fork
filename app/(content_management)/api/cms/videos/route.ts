...
import { getVideos, createVideo } from "@/lib/cms/service";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const items = await getVideos({ region: region ?? undefined, status, limit });
...
export async function POST(req: NextRequest) {
  try {
    ...
    const msg = e instanceof Error ? e.message : "Failed to create videos";
